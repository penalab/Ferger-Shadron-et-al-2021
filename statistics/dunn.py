...
def kw_dunn(groups, to_compare=None, alpha=0.05, method='bonf'):
    ...
    we_see_ties = np.zeros(ncomp, dtype=np.bool)
    Rmean = R / n
    ...
    for pp, (ii, jj) in enumerate(to_compare):
        ...
        if sum([x>1 for x in ts3_ts]) > 0:
            ...
            theta10 = (N * (N + 1.0)) / 12.0
            theta11 =  E_ts3_ts / ( 12.0 * (N - 1) )
...
#    print N, n
    ...
    return H, p_omnibus, Z_pairs, p_corrected, reject, p_uncorrected, we_see_ties
